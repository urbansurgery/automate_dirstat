...
class Report:
    @staticmethod
    def generate_pdf(all_densities: List[float],
                     all_areas: List[float],
                     data: List[List[Union[str, float, int]]],
                     threshold: float,
                     summary_data=None) -> IO[bytes]:
        """
        ...
        available_width = A4[0] - 2 * 72  # A4[0] gives the width of the A4 page in points
        ...
        doc = SimpleDocTemplate(pdf_buffer, pagesize=portrait(A4), rightMargin=72, leftMargin=72, topMargin=72,
                                bottomMargin=128)
        story = []
        ...
        if summary_data is not None:
            ...
            summary_table.setStyle(TableStyle([
                ('TEXTCOLOR', (1, 6), (1, 7), result_color)  # Targeting only the "Result" cell
            ]))
        ...
        plot_buffer_density.seek(0)
        width, height = Report.get_resized_dimensions(plot_buffer_density, target_width=available_width,
        ...
        story.append(Paragraph("Correlation between Area and Density:", styles['Heading2']))
        plot_buffer_correlation.seek(0)
        width, height = Report.get_resized_dimensions(plot_buffer_correlation, target_width=available_width,
                                                      max_height=available_width)
    ...
    @staticmethod
    def generate_summary(threshold: float, pass_rate_percentage: float, health_objects: Dict[str, HealthObject],
                         commit_details: Dict[str, str]) -> Dict[str, Any]:
        # Calculate the number of objects above the threshold
        above_threshold_count = sum(1 for ho in health_objects.values() if ho.aggregate_density > threshold)
